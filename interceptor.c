...
void my_exit_group(int status){
     	...
     	del_pid(current->pid);//removes the pid from all syscall lists
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	asmlinkage long (*original)(struct pt_regs);

	long sysnum = reg.ax;
	long return_val;

	spin_lock(&my_table_lock);
  	if(table[sysnum].monitored == 2 || check_pid_monitored(sysnum, current->pid) == 1){ //pid is monitored for this sysnum
      		//log message
      		log_message(current->pid, sysnum, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
  	}
	original = table[sysnum].f;
        spin_unlock(&my_table_lock); 
		
        return_val = (*(original))(reg); //call original syscall
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT){
	   ...
	   table[syscall].f = sys_call_table[syscall];//set the original pointer
	   sys_call_table[syscall] = interceptor; //set the handler to be the interceptor
	   ...
	   spin_unlock(&my_table_lock); 
          ...
          //de-intercept the 'syscall' argument
	 ...
	 sys_call_table[syscall] = table[syscall].f;//reset the original pointer
	 table[syscall].intercepted = 0;
	 table[syscall].monitored = 0;
	 destroy_list(syscall); //clear the set of monitored pids
	 ...
	 spin_unlock(&my_table_lock); 
   	    ...
   	    // start monitoring for 'pid' whenever it issues 'syscall'
	    ...
	    spin_lock(&my_table_lock);
            if(table[syscall].intercepted == 0 || table[syscall].monitored == 0 || (pid != 0 && check_pid_monitored(syscall, pid) == 0))  {
                //pid is not monitored for this syscall
		     spin_unlock(&my_table_lock); 
                return -EINVAL;
            }
		...
		destroy_list(syscall);
                ...
                if(table[syscall].listcount == 0){
                    //this was the only pid that was monitored
                    table[syscall].monitored = 0;
                }
            }
...
static int init_function(void) {
    ...
    //intercept custom syscall with my_syscall

    orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];//save the original pointer
    sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall; //set the handler to be my_syscall

    //intercept exit
    orig_exit_group = sys_call_table[__NR_exit_group];//save the original pointer
    sys_call_table[__NR_exit_group] = my_exit_group; //set the handler to be my_exit_group
...
static void exit_function(void)
{
    ...
    for(i = 1; i < NR_syscalls; i++) //syscall 0 is custom function so start at 1
    {
    ...
    //restore custom and exit syscalls
    sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall; //restore the original
    sys_call_table[__NR_exit_group] = orig_exit_group; //restore the original
